...
export interface ReadAndServeResult {
  ...
  served: ServedRow[];
  /** The served rows as ordered records for programmatic consumers. */
  records: ReadRecord[];
  /** The complete logical line count before pagination/truncation. */
  totalLines: number;
  /** The next 1-based offset when another page exists. */
  nextOffset?: number;
  diagnostics?: string[];
...
export async function readAndServe(
  ...
  const warning = promotionWarning
    ? autoWarning
      ...
      : promotionWarning
    : autoWarning;
  const lines = view.normalized.length === 0 ? [] : splitLines(view.normalized);
  const records = view.served.map((row) => ({
    line: row.position + 1,
    hash: row.hash,
    content: lines[row.position] ?? "",
  }));
  const totalLines = lines.length;
  const text = view.hadUtf8DecodeErrors ? `${view.text}\n\n${UTF8_REWRITE_NOTE}` : view.text;
  const diagnostics = [
    ...(view.diagnostics ?? []),
    ...(view.hadUtf8DecodeErrors ? [UTF8_REWRITE_NOTE] : []),
  ];
  return {
    text,
    served: view.served,
