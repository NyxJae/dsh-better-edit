...
import type { RMeta, BatchSection, StructuredEditResult } from "./edit-response.js";
...
export async function execute(opts: {
  ...
  sandbox: FsSandboxController;
  sandboxPolicy: SandboxExecutionPolicy | undefined;
  resultFormat?: "text" | "structured";
}): Promise<string | StructuredEditResult> {
  const {
    io,
    items,
    sessionKey,
  ...
  const formatResult = (
    built: ReturnType<typeof buildBatchResult>,
  ): string | StructuredEditResult =>
    resultFormat === "structured" ? toStructuredResult(built) : built.content[0]!.text;
