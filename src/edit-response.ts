...
function formatDiffCard(diff: string): string {
  ...
  const hint = truncation.firstLineExceedsLimit
...
export function buildChanged(input: SuccessInput): TResult {
  ...
  const freshRows: FreshRow[] = diffResult.servedRows.map((row) => ({
    line: row.position + 1,
    hash: row.hash,
    content: resultLines[row.position] ?? "",
  }));
  return {
    content: [{ type: "text", text }],
    details: {
...
export function buildBatchResult(sections: BatchSection[]): TResult {
  ...
  for (const s of appliedFiles) {
    ...
    const resultLines = visLines(s.result);
    for (const row of diffResult.servedRows) {
      freshRows.push({
        line: row.position + 1,
        hash: row.hash,
        content: resultLines[row.position] ?? "",
...
export function toStructuredResult(result: TResult): StructuredEditResult {
  const details = result.details;
  const metrics = details.metrics ?? {
    edits_attempted: 0,
    edits_noop: 0,
    warnings: 0,
    classification: details.classification ?? "noop",
  };
  const classification = details.classification ?? metrics.classification;
  return {
    format: "structured",
    classification,
    metrics,
    fresh_rows: details.freshRows ?? [],
    ...(details.warnings && details.warnings.length > 0 ? { warnings: details.warnings } : {}),
    ...(details.driftNotice ? { drift_notice: details.driftNotice } : {}),
  };
}
